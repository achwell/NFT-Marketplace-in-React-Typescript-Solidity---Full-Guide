...
export type Web3State = { isLoading: boolean, hooks: Web3Hooks } & Nullable<Web3Dependencies>
...
export const loadContract = async (name: string, provider: providers.Web3Provider): Promise<Contract> => {
    ...
    if (Artifact.networks[NETWORK_ID].address) {
        return new ethers.Contract(Artifact.networks[NETWORK_ID].address, Artifact.abi, provider)
    } else {
